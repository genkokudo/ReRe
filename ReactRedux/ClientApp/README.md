このプロジェクトは、[Create React App](https://github.com/facebookincubator/create-react-app)でブートストラップされました。

以下に、一般的なタスクの実行方法に関する情報を示します。<br>
このガイドの最新版は[こちら](https://github.com/facebookincubator/create-react-app/blob/master/packages/react-scripts/template/README.md)でご覧いただけます。

## 目次

- [新しいリリースへの更新](#updating-to-new-releases)
- [フィードバックを送信する](#sending-feedback)
...
- [エディターでのリント出力の表示](#displaying-lint-output-in-the-editor)
- [エディターでのデバッグ](#debugging-in-the-editor)
- [コードの自動フォーマット](#formatting-code-automatically)
- [ページを変更する `<title>`](#changing-the-page-title)
- [依存関係のインストール](#installing-a-dependency)
- [コンポーネントのインポート](#importing-a-component)
- [コード分割](#code-splitting)
- [スタイルシートを追加する](#adding-a-stylesheet)
- [後処理CSS](#post-processing-css)
- [CSSプリプロセッサの追加（Sass、Lessなど）](#adding-a-css-preprocessor-sass-less-etc)
- [画像、フォント、ファイルを追加する](#adding-images-fonts-and-files)
- [publicフォルダーを使用する](#using-the-public-folder)
  - [HTMLを変更する](#changing-the-html)
  - [モジュールシステム外でのアセットの追加](#adding-assets-outside-of-the-module-system)
  - [publicフォルダーを使用する場合](#when-to-use-the-public-folder)
- [グローバル変数の使用](#using-global-variables)
- [Bootstrapの追加](#adding-bootstrap)
  - [カスタムテーマの使用](#using-a-custom-theme)
- [フローを追加する](#adding-flow)
- [カスタム環境変数の追加](#adding-custom-environment-variables)
  - [HTMLでの環境変数の参照](#referencing-environment-variables-in-the-html)
  - [シェルに一時的な環境変数を追加する](#adding-temporary-environment-variables-in-your-shell)
  - [開発環境変数の追加 `.env`](#adding-development-environment-variables-in-env)
- [デコレータを使用できますか？](#can-i-use-decorators)
- [APIバックエンドとの統合](#integrating-with-an-api-backend)
  - [Node](#node)
  - [Ruby on Rails](#ruby-on-rails)
- [開発中のAPIリクエストのプロキシ](#proxying-api-requests-in-development)
  - [プロキシの構成後の「無効なホストヘッダー」エラー](#invalid-host-header-errors-after-configuring-proxy)
  - [プロキシを手動で構成する](#configuring-the-proxy-manually)
  - [WebSocketプロキシの構成](#configuring-a-websocket-proxy)
- [開発でのHTTPSの使用](#using-https-in-development)
- [サーバーでの動的`<meta>`タグの生成](#generating-dynamic-meta-tags-on-the-server)
- [静的HTMLファイルへの事前レンダリング](#pre-rendering-into-static-html-files)
- [サーバーからページへのデータの注入](#injecting-data-from-the-server-into-the-page)
- [テストの実行](#running-tests)
  - [ファイル名の規則](#filename-conventions)
  - [コマンドラインインターフェース](#command-line-interface)
  - [バージョン管理の統合](#version-control-integration)
  - [テストを書く](#writing-tests)
  - [コンポーネントのテスト](#testing-components)
  - [サードパーティアサーションライブラリの使用](#using-third-party-assertion-libraries)
  - [テスト環境の初期化](#initializing-test-environment)
  - [テストの焦点と除外](#focusing-and-excluding-tests)
  - [カバレッジレポート](#coverage-reporting)
  - [継続的インテグレーション](#continuous-integration)
  - [jsdomを無効にする](#disabling-jsdom)
  - [スナップショットテスト](#snapshot-testing)
  - [エディター統合](#editor-integration)
- [分離したコンポーネントの開発](#developing-components-in-isolation)
  - [Storybookの使用開始](#getting-started-with-storybook)
  - [Styleguidistの使用開始](#getting-started-with-styleguidist)
- [プログレッシブWebアプリの作成](#making-a-progressive-web-app)
  - [キャッシングのオプトアウト](#opting-out-of-caching)
  - [オフライン優先の考慮事項](#offline-first-considerations)
  - [プログレッシブWebアプリのメタデータ](#progressive-web-app-metadata)
